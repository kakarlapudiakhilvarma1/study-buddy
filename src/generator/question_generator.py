...
class QuestionGenerator:
        ...
        for attempt in range(settings.MAX_RETRIES):
            try:
                ...
                if attempt==settings.MAX_RETRIES-1:
                    raise CustomException(f"Generatio failed after {settings.MAX_RETRIES} attempts",e)
    ...
    def generate_fill_blank(self,topic:str,difficulty:str='medium') -> FillBlankQuestion:
        try:
            ...
            self.logger.info("Generated a valid FillBlank Question")

            return question
        
        except Exception as e:
            self.logger.error(f"Failed to generate fillups : {str(e)}")
            raise CustomException("FillBlank generation failed",e)
